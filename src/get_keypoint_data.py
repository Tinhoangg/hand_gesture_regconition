...
def get_neck_point(input_video):
    cap = cv2.VideoCapture(input_video)  # đường dẫn video

    pose = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.6, min_tracking_confidence=0.5)
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            mp_drawing_pose.draw_landmarks(frame, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)
    pose.close()
...
def get_keypoint(input_video, output_path):
    frame_count = 0
    ...
    neck_point, shoulder_dist = get_neck_point(input_video)
    ...
    with mp_hands.Hands(static_image_mode=False,
        ...
        while True:
            ...
            normalized_kp = normalize_keypoints(keypoints_frame, neck_point, shoulder_dist)
            all_keypoints.append(normalized_kp)

    cap.release()
    all_keypoints = np.array(all_keypoints)
    np.save(output_path, all_keypoints)
    print(f"Đã lưu keypoints vào {output_path}, shape = {all_keypoints.shape}")
        ...
        if os.path.basename(root):
            ...
            for file in files:
                if file.endswith(".mp4"):
                    ...
                    os.makedirs(f"keypoint/{parent_folder}", exist_ok=True)
                    save_path = f'keypoint/{parent_folder}/{parent_folder}_{count}.npy'
                    get_keypoint(full_path, save_path)  
                    print('label: ',parent_folder, 'number: ', count)
                    count += 1
